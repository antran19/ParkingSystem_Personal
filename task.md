...
- [x] Tích hợp FE + BE (test luồng check-in/out end-to-end) ✅
...
- [x] Form: nhập biển số (hoặc giả lập camera OCR)
- [x] Chọn loại xe (xe máy / ô tô)
- [x] Hiển thị zone được gợi ý (gọi API)
- [x] Khi confirm → tạo session → hiển thị QR code
- [x] Phân biệt 3 loại driver khi check-in:
  - Vãng lai → nhập biển số, scan mới
  - Đặt trước → nhập mã reservation
  - Có vé → scan biển số, tự nhận diện

**Staff Check-out v2 (ưu tiên cao):**
- [x] Quét QR hoặc nhập biển số
- [x] Hiện: thời gian gửi, loại xe, zone, giá tiền
- [x] Chọn phương thức thanh toán (tiền mặt / chuyển khoản)
- [x] Confirm → session completed
...
### Phải có:
- [x] Login 5 role
- [x] Staff check-in xe vãng lai (cổng chính → cổng tầng)
- [x] Hệ thống gợi ý zone còn chỗ
- [x] Sinh QR cho session (mã hóa session code)
- [x] Staff check-out (cổng tầng → cổng chính → thanh toán)
- [x] Tính phí tự động
